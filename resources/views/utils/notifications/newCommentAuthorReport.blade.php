<h5 class="mb-2 is-700"><i class="fas fa-plus fa-fw"></i><i class="fas fa-comments fa-fw"></i> ¡Nuevo comentario en un reporte tuyo!</h5>
<p class="my-1 text-smaller">Han hecho un nuevo comentario en tu reporte de <i class="{{$notification->data['report']['icon']}} fa-fw"></i> {{$notification->data['report']['label']}} <b>"{{$notification->data['report']['title']}}"</b>.</p>
<p class="my-3 ml-3 text-smallest bg-light p-3 rounded">
  <b>{{$notification->data['comment']['author']}}</b><br/>
  {!! nl2br(e($notification->data['comment']['content'])) !!}
</p>
