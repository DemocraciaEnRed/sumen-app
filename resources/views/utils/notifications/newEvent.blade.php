...
<p class="my-1 text-smaller">Hay un nuevo evento llamado <b>{{$notification->data['event']['title']}}</b> relacionado con una meta al que estas suscripto. Podes ver mas acerca del evento haciendo <a href="{{route('events.index', ['eventId' => $notification->data['event']['id']])}}" >click aquí</a></p>
