<h5 class="mb-2 is-700"><i class="fas fa-medal fa-fw"></i><i class="far fa-laugh-beam fa-fw"></i> ¡Un proyecto llegó a su 100%!</h5>
<p class="my-1 text-smaller">¡Estamos celebrando porque el proyecto <b>"{{$notification->data['goal']['title']}}"</b> llegó a su 100%! Podes leer el nuevo reporte de {{$notification->data['report']['label']}} <b>"{{$notification->data['report']['title']}}"</b> haciendo <a href="{{route('reports.index', ['reportId' => $notification->data['report']['id']])}}" >click aquí</a></p>
